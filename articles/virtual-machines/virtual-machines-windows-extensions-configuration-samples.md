<properties
   pageTitle="Konfigurace Ukázka rozšíření OM Windows | Microsoft Azure"
   description="Ukázka konfigurace pro vytváření šablon s příponou"
...
# <a name="azure-windows-vm-extension-configuration-samples"></a>Azure Windows OM rozšíření konfigurace vzorky

> [AZURE.SELECTOR]
- [Prostředí PowerShell – šablony](virtual-machines-windows-extensions-configuration-samples.md)
- [Rozhraní příkazového řádku - šablony](virtual-machines-linux-extensions-configuration-samples.md)

<br>

Tento článek obsahuje ukázkové konfigurace pro konfiguraci Azure OM rozšíření pro Windows VMs.

Další informace o těchto rozšíření najdete v tématu [Azure OM rozšíření Přehled.](virtual-machines-windows-extensions-features.md)

Další informace o vytváření rozšíření šablon najdete v tématu [pro vytváření šablon rozšíření.](virtual-machines-windows-extensions-authoring-templates.md)

Tento článek obsahuje očekávané konfigurace hodnoty pro některé doplňky systému Windows.

## <a name="sample-template-snippet-for-vm-extensions-with-iaas-vms"></a>Ukázka šablony fragment pro rozšíření OM s IaaS VMs.
Šabloně fragment pro nasazení rozšíření vypadá jako následující:
...
## <a name="sample-template-snippet-for-vm-extensions-with-vm-scale-sets"></a>Ukázka šablony fragment pro rozšíření OM sadami OM měřítko.
...
Před nasazením rozšíření zkontrolujte nejnovější verze rozšíření a "typeHandlerVersion" nahradit aktuální nejnovější verzi.

Zbytek v článku obsahuje ukázkové konfigurace pro rozšíření OM Windows.
...
- fileUris: čárkou oddělení číslic seznam adres URL soubory, které bude možné stáhnout na OM rozšíření. Pokud není nic uvedeno stahování žádné soubory. Pokud jsou soubory v úložišti Azure, fileURLs můžete označit jako soukromý a correspoding storageAccountName a storageAccountKey můžete předané jako soukromé parametry přístup k těmto souborům.
- commandToExecute: [povinné parametr]: Toto je příkaz, který bude proveden příponu.
- storageAccountName: [volitelné parametr]: název účtu úložiště pro přístup k fileURLs, pokud jsou označeny jako soukromé.
- storageAccountKey: [volitelné parametr]: klíč účtu úložiště pro přístup k fileURLs, pokud jsou označeny jako soukromé.

### <a name="customscript-extension-17"></a>Rozšíření CustomScript 1.7.

Získáte CustomScript verze 1.4 parametr popis. Verze 1.7 přináší podporu odesílání skript parameters(commandToExecute) jako protectedSettings, v takovém případě se budou šifrovány před odesláním. parametr "commandToExecute" může být zadán v nastavení nebo protectedSettings ale ne v obou.
...
### <a name="dsc-extension"></a>DSC rozšíření.
      {
...
### <a name="trend-micro-deep-security-agent"></a>Agent malých důkladné zabezpečení trendu.
      {
...
### <a name="vormertric-transparent-encryption-agent"></a>Agent Vormertric průhledné šifrování.
            {
              "publisher": "Vormetric",
...
### <a name="puppet-enterprise-agent"></a>Agent puppet organizace.
            {
...
### <a name="microsoft-monitoring-agent-for-azure-operational-insights"></a>Microsoft sledování Agent pro Azure provozní přehledy
            {
...
### <a name="eset-file-security"></a>Zabezpečení souborů obnovit
          {
            "publisher": "ESET",
            "type": "FileSecurity",
            "typeHandlerVersion": "6.0",
...
### <a name="confer-advanced-threat-prevention-and-incident-response-for-azure"></a>Přenést Upřesnit hrozbou prevence a vyřešení incidentu odpověď pro Azure
          {
            "publisher": "Confer",
            "type": "ConferForAzure",
            "typeHandlerVersion": "1.0",
...
### <a name="barracuda-vpn-connectivity-agent-for-microsoft-azure"></a>Agent připojení VPN barracuda pro Microsoft Azure
          {
...
### <a name="alert-logic-log-manager"></a>Upozornit správce LOGZ logiky
          {
...
Další informace o tom, jak konfigurovat diagnostiky najdete v článku [Rozšíření diagnostiky Azure](virtual-machines-windows-extensions-diagnostics-template.md)
...
V těchto příkladech nahraďte číslo verze číslo nejnovější verzi.

Tady je příklad úplné OM šablona s příponou vlastní skript.
