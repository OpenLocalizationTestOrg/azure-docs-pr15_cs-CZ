<properties
   pageTitle="Ukázka životního cyklu aplikace založené na ZBÝVAJÍCÍ | Microsoft Azure"
   description="Ukázka struktury služby Microsoft Azure zobrazující životním cyklu aplikace pomocí rozhraní REST struktury služby."
...
# <a name="rest-based-application-lifecycle-sample"></a>Ukázka životního cyklu na základě ZBÝVAJÍCÍ aplikace

Tento příklad ukazuje životním cyklu aplikace služby struktury prostřednictvím rozhraní REST API volání. Další informace o životním cyklu aplikace služby struktury najdete v článku [životního cyklu služby struktury aplikace](service-fabric-application-lifecycle.md).

V tomto příkladu provede následující:

* Ustanovení obsahují ukázku **WordCount 1.0.0** z balíčku aplikace WordCount v obraze.
* Seznam typů aplikace, která zahrnuje WordCount 1.0.0.
* Vytvoří aplikaci WordCount jako **struktury: / WordCount**.
* Zobrazí v seznamu aplikací, který obsahuje struktury: / WordCount verze 1.0.0.
* Verze ustanovení 1.1.0 vzorku WordCount z balíčku aplikace **WordCountUpgrade** v úložišti obrázek.
* Seznam typů aplikací, který obsahuje WordCount 1.0.0 a **WordCount 1.1.0**.
* Upgrade aplikace WordCount verzi 1.1.0.
* Zobrazí v seznamu aplikací, který obsahuje WordCount verze 1.1.0, ale již obsahuje WordCount verze 1.0.0.
* Odstraní WordCount aplikace.
* Zobrazí v seznamu aplikací, který již obsahuje struktury: / WordCount.
* Verze unprovisions 1.1.0 WordCount vzorku.
* Seznam typů aplikace, která obsahuje WordCount 1.0.0, ale již obsahuje WordCount 1.1.0.
* Verze unprovisions 1.0.0 WordCount vzorku.
* Seznam typů aplikací, který již obsahuje WordCount.


## <a name="prerequisites"></a>Zjistit předpoklady pro

V tomto příkladu [WordCount ukázka](http://aka.ms/servicefabricsamples) (najdete ve vzorcích **Začínáme** ). Ukázka WordCount musíte nejdřív integrované a potom musí dvou balíčků aplikací Kopírovat k úložišti obrázek.

|Složka|Popis|
|------|-----------|
|WordCount|Použití vzorku WordCount. Soubor **ApplicationManifest.xml** obsahuje **ApplicationTypeVersion = "1.0.0"**.|
|WordCountUpgrade|Použití vzorku WordCount. Soubor ApplicationManifest.xml musí změnit na **ApplicationTypeVersion = "1.1.0"** povolit upgrade aplikace problémy.|

Pokud chcete vytvořit balíčků aplikací a zkopírujte do úložiště obrázek, proveďte následující kroky:

1. Zkopírujte **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** **C:\Temp\WordCount**. Tím vytvoříte balíčku aplikace WordCount.
2. Zkopírujte C:\Temp\WordCount **C:\Temp\WordCountUpgrade**. Tím vytvoříte balíček **aplikace WordCountUpgrade** .
3. Otevřete **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** v textovém editoru.
4. V elementu **ApplicationManifest** změňte atribut **ApplicationTypeVersion** na **"1.1.0"**.  Toto tlačítko aktualizujete číslo verze aplikace.
5. Uložte soubor změněné ApplicationManifest.xml.
6. Tento skript Powershellu spustili s oprávněními správce zkopírujte aplikací k úložišti obrázek:
...
Po dokončení skript Powershellu je připravená k spuštění této aplikace.

## <a name="example"></a>Příklad

Následující příklad ukazuje životním cyklu aplikace služby struktury.
