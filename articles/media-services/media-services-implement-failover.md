<properties 
    pageTitle="Provádění převzetí streamování scénář | Microsoft Azure" 
    description="Toto téma ukazuje, jak implementovat převzetí streamování scénář." 
...
#<a name="implementing-failover-streaming-scenario"></a>Provádění převzetí streamování scénářů

Tento návod ukazuje, jak kopírování obsahu (objektů BLOB) z jednoho majetku do jiného za účelem zpracování redundance na vyžádání datového proudu. Tento scénář je užitečný pro zákazníky, které chcete nastavit jejich CDN převzetí mezi dvěma datacentrech v případě výpadku v jednom z našich datacentrech.
Tento návod používá Microsoft Azure Media Services SDK, Microsoft Azure Media Services REST API a Azure úložiště SDK prokázat úkoly podle těchto pokynů.

1. Nastavení účtu služby médií v "Datacentra A".
1. Nahrání souboru mezzanine do majetku zdroje.
1. Kódování majetku do vícenásobného bit sazba MP4 souborů. 
1. Vytvoření jen pro čtení locator přidružení zabezpečení pro zdroj materiály ke čtení do kontejneru úložiště účtu, který bude přidružený materiálů zdroje.
1. Zjištění názvu kontejneru materiálů zdroje z locator přidružení zabezpečení jen pro čtení vytvořili v předchozím kroku. Potřebujeme tyto informace ke kopírování objektů BLOB mezi účty úložiště (vysvětleno dále v tématu).
1. Vytvoření origin locator majetku vytvořil kódování úkolu. 

Chcete-li potom zpracovat záložní:

1. Nastavení účtu služby médií v "Dat centra B".
1. Vytvořte prázdný majetek cílové v cílové Media Services účtu.
1. Vytvoření locator přidružení zabezpečení zápisu prázdné položka cílové přístup pro zápis do kontejneru v cílové úložiště účet, který souvisí s cílovou materiálů.
1. Slouží ke kopírování objektů BLOB (soubory majetku) mezi zdroj úložiště v "Datacentra A" a cílového úložiště účet v "Dat centra B" Azure úložiště SDK (tyto účty úložiště nejsou přidružené prostředky potřebné.)
1. Přiřazení objektů BLOB (soubory majetku), které byly zkopírovány do cílového kontejneru objektů blob s cílovou materiálů. 
1. Vytvoření origin locator položka v "Dat centra B" a zadejte URL Id, který byl vytvořen položka v "Datacentra A". 
1. To vám dá streamování adresy URL kde relativní cesty adresy URL jsou stejná (pouze základu adres URL se liší). 
 
Pak pro zpracování všech výpadků, můžete vytvořit CDN nad tyto Locator origin. 

Platí následující omezení:

- Aktuální verze Media Services SDK nepodporuje vytváření locator s id zadané locator. Abyste dosáhli tento úkol, použijeme Media Services REST API.
- Aktuální verze Media Services SDK nepodporuje programově generování IAssetFile informace, které chcete přidružit k aktivum materiálů soubory. Abyste dosáhli tento úkol, použijeme CreateFileInfos Media Services REST API. 
- Úložiště zašifrovaných aktiva (AssetCreationOptions.StorageEncrypted) nejsou podporovaná replikace (protože šifrovacího klíče budou lišit v obou Media Services účty). 
- Pokud chcete využít dynamické balení, musíte nejprve získat alespoň jeden na vyžádání streamování rezervovaná jednotky. Další informace najdete v tématu [Dynamicky balení prostředky](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Zvažte použití Media Services [Replikace nástroj](http://replicator.codeplex.com/) jako alternativu k provádění streamování situace převzetí ručně. Tento nástroj umožňuje replikace prostředky dva účty Media Services.

##<a name="prerequisites"></a>Zjistit předpoklady pro
 
- Dva účty Media Services v nové nebo existující Azure předplatného. Zjistěte, [jak vytvořit účet služby média](media-services-portal-create-account.md).
- Operační systémy: Windows 7, Windows 2008 R2 nebo Windows 8.
- .NET framework 4.5 nebo .NET Framework 4.
- Visual Studio 2010 SP1 nebo novější (Professional, Premium, Ultimate nebo Express).
 
##<a name="set-up-your-project"></a>Nastavení projektu

V této části vytvoříte a nastavení aplikace konzoly C# projektu.

1. Umožňuje vytvořit nové řešení, která obsahuje projekt C# konzoly aplikace Visual Studio. Zadejte HandleRedundancyForOnDemandStreaming pro název a potom klikněte na OK.
1. Vytvoření složky SupportFiles na stejné úrovni jako soubor HandleRedundancyForOnDemandStreaming.csproj projektu. Ve složce SupportFiles vytvořte složky OutputFiles a MP4Files. Zkopíroval soubor s příponou .mp4 do složky MP4Files (v tomto příkladě se používá BigBuckBunny.mp4 soubor). 
...
1. Přidání dalších odkazů, které jsou zapotřebí pro tento projekt: System.Configuration System.Runtime.Serialization a System.Web.
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Přidáte kód, který zpracovává redundance na vyžádání datového proudu.



1. Přidejte následující pole úrovni třídy třídy Program.
...
1. Definice výchozího hlavní metody nahraďte následující:
...
1. Definice metody, které se označují jako z hlavní jsou definovány níže.
...
Teď můžete přenosy odpovědi na žádosti o směrování mezi dvěma datacentrech a tedy převzetí v případě jakékoli výpadků.
