<properties 
   pageTitle="Ovládací prvek směrování a používat virtuální zařízení ve Správci zdrojů pomocí šablony | Microsoft Azure"
   description="Zjistěte, jak řídí směrování a virtuální zařízení v Azure správce prostředků použít pomocí šablony"
...
#<a name="create-user-defined-routes-udr-in-resource-manager-by-using-a-template"></a>Vytvoření uživatele definované směruje (UDR) ve Správci zdrojů pomocí šablony
...
## <a name="udr-resources-in-a-template-file"></a>UDR zdrojů v souboru šablony

Můžete zobrazit a stáhnout [ukázkové šablony](https://github.com/telmosampaio/azure-templates/tree/master/IaaS-NSG-UDR).

Oddíl níž znázorňuje definici front-end UDR v souboru **azuredeploy vnet nsg udr.json** podle výše uvedených scénáře.
...
Chcete-li přidružit UDR podsítě front-end, budete muset změnit definici podsítě v šabloně a použít id odkazu UDR.
...
Taky musíte zajistit, aby OM **FW1** IP přesměrování povolenou na NIC, která se bude používat pro příjem a předávání paketů. V části ukazuje definici NIC FW1 v souboru azuredeploy nsg udr.json podle výše uvedených scénáře.
...
2. Spustit `New-AzureRmResourceGroup` rutina vytvořit skupinu zdrojů.

        New-AzureRmResourceGroup -Name TestRG -Location westus

3. Spustit `New-AzureRmResourceGroupDeployment` rutina zavést šablonu.
...
3. V prohlížeči přejděte na **https://raw.githubusercontent.com/telmosampaio/azure-templates/master/IaaS-NSG-UDR/azuredeploy.parameters.json**, zkopírujte obsah souboru json a vložte nového souboru v počítači. V tomto scénáři by být kopírování hodnot menších než do souboru s názvem **c:\udr\azuredeploy.parameters.json**.
...
4. Spusťte rutinu **azure skupina vytvářet** nasazení nové VNet pomocí šablony a parametr soubory můžete stáhnout a kterou nad. Seznam zobrazený po výstup vysvětluje parametry použité.
...
5. Příkaz **Zobrazit azure skupiny** zobrazíte prostředků vytvořené v nové skupiny prostředků. 
...
>[AZURE.TIP] Pokud nevidíte všechny zdroje, spusťte `azure group deployment show` příkaz zajistit zřizovací stav nasazení je *Succeded*.