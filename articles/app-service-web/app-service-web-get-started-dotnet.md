<properties 
    pageTitle="Nasazení webovou aplikaci první .NET Azure pět minut | Microsoft Azure" 
...
# <a name="deploy-your-first-net-web-app-to-azure-in-five-minutes"></a>Nasazení webovou aplikaci první .NET Azure pět minut

Tento kurz vám pomůže publikovat do jednoduchých webových aplikací .NET pro [Aplikaci služby Azure](../app-service/app-service-value-prop-what-is.md).
...
## <a name="deploy-an-net-web-app"></a>Nasazení webové aplikace pro .NET
...
1. Změna pracovního adresáře (`CD`) a klonovat aplikaci vzorku.
...
4. Vytvoření zdrojů aplikace aplikaci služby v Azure s názvem jedinečné aplikace a nasazení uživatele, která odrážejí dříve konfigurované. Pokud se zobrazí výzva, zadejte počet na požadovanou oblast.
    ...
    ![Vytvoření Azure prostředků pro webovou aplikaci první v Azure](./media/app-service-web-get-started-languages/dotnet-site-create.png)
...
4. Nasazení ukázkový kód Azure aplikace, jako by push jakýkoli kód s libovolná. Po zobrazení výzvy: použijte heslo, která odrážejí dříve konfigurované.
    ...
    ![Použít kód pro první webovou aplikaci v Azure](./media/app-service-web-get-started-languages/dotnet-git-push.png)

    `git push`nejen umístí kód Azure, ale taky obnoví vyžadované balíčky a vytvoří binární ASP.NET. 
...
Zjistěte, jak vytvořit, vytvoření a nasazení Azure přímo ve Visual Studiu na [nasazení aplikace ASP.NET webové aplikace služby Azure, pomocí aplikace Visual Studio](web-sites-dotnet-get-started.md).NET webových aplikacích.
